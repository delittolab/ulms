...
OUTPUT_MASTER_DIR = jpascvi.create_output_dir(PARENT_DIR, 'scANVI_tumor_hvg')
...
N_TOP_GENES = 200 # number of highly variables genes in the G4X data. Subset to these genes in G4X and scRNAseq data
...
print(adata_g4x.n_vars)
gene_list = adata_g4x.var_names.tolist()
...
print(adata_g4x.obs['cell_type'])
print(np.unique(adata_g4x.obs['cell_type']))
adata_g4x = adata_g4x[adata_g4x.obs['cell_type'] != 'Necrosis'].copy()
adata_g4x.obs['cell_type'] = adata_g4x.obs['cell_type'].cat.remove_unused_categories()
adata_g4x.obs['cell_type'] = adata_g4x.obs['cell_type'].cat.rename_categories(
    {'Macrophage': 'Myeloid'}
)
print(np.unique(adata_g4x.obs['cell_type']))
print(adata_g4x)
...
# Find the most informative genes in the G4X tumor cells
# Maybe this will denoise and help with prediction
sc.pp.filter_genes(adata_g4x, min_cells=3)
span = 0.3
while True:
    try:
        sc.pp.highly_variable_genes(adata_g4x,
...
adata_g4x.obs[cell_type_key] = 'Unknown'
adata_g4x.obs[cell_type_key] = adata_g4x.obs[cell_type_key].astype('category')

print(f"G4X anndata after subsetting to {N_TOP_GENES}:")
print(adata_g4x)
...
prob_df.to_csv(SCANVI_FULL_DIR / 'full_predictions.h5ad')
